...
class AreaExcelImportService
{
     ...
     *     row_count: int,
     *     valid_rows: int,
     *     rows: list<array{excel_row:int,code:string,name_ar:string,city:?string,status:string,notes:?string}>,
     *     errors: list<string>
     * }
     */
        ...
        try {
            ...
            for ($excelRow = 2; $excelRow <= $highestRow; $excelRow++) {
                ...
                $row = [
                    ...
                    'city' => $this->nullableString($values[2]),
                ...
                $validator = Validator::make(
                    $row,
                    [
                        'code' => ['required', 'string', 'max:255', Rule::unique('areas', 'code')],
                    ...
                    [
                        'code.required' => 'رمز المنطقة مطلوب.',
                        'code.max' => 'رمز المنطقة لا يجوز أن يتجاوز 255 محرفًا.',
                        'code.unique' => 'رمز المنطقة موجود مسبقًا في النظام.',
                        'name_ar.required' => 'اسم المنطقة مطلوب.',
                        'name_ar.max' => 'اسم المنطقة لا يجوز أن يتجاوز 255 محرفًا.',
                        'city.max' => 'المدينة / المحافظة لا يجوز أن تتجاوز 255 محرفًا.',
                ...
                if ($normalizedCode !== '' && isset($seenCodes[$normalizedCode])) {
                    $rowErrors[] = 'رمز المنطقة مكرر داخل ملف Excel نفسه (ظهر أولًا في الصف '.$seenCodes[$normalizedCode].').';
