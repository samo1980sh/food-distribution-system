@php
    ...
    if ($uploadedFile instanceof TemporaryUploadedFile) {
        $preview = app(ProductCategoryExcelImportService::class)->analyze(
...
@if ($preview !== null)
    ...
    <style>
        .fd-product-category-import-preview {
            --fd-border: #e5e7eb;
            --fd-soft: #f8fafc;
            --fd-text: #111827;
            --fd-muted: #64748b;
        ...
        .dark .fd-product-category-import-preview {
        ...
        .fd-product-category-import-preview * { box-sizing: border-box; }
        ...
        .fd-import-table { width: 100%; min-width: 900px; border-collapse: collapse; table-layout: fixed; }
        .fd-import-table th { padding: 10px 12px; border-bottom: 1px solid var(--fd-border); background: var(--fd-soft); text-align: right; font-size: 11px; font-weight: 700; color: var(--fd-muted); white-space: nowrap; }
        .fd-import-table td { padding: 11px 12px; border-bottom: 1px solid var(--fd-border); vertical-align: middle; color: var(--fd-text); line-height: 1.5; }
        ...
        .fd-col-code { width: 155px; }
        .fd-col-parent { width: 155px; }
        .fd-col-sort { width: 105px; }
        .fd-col-status { width: 110px; }
        ...
        .fd-status { display: inline-flex; align-items: center; justify-content: center; min-width: 64px; border-radius: 999px; padding: 5px 9px; font-size: 11px; font-weight: 700; }
        .fd-status.is-active { background: var(--fd-green-soft); color: var(--fd-green); }
        .fd-status.is-inactive { background: var(--fd-soft); color: var(--fd-muted); }
    ...
    <div class="fd-product-category-import-preview">
        <div class="fd-import-shell">
            <div class="fd-import-head">
                <div>
                    <h3 class="fd-import-title">نتيجة فحص الملف</h3>
                    <p class="fd-import-subtitle">تم تحليل التصنيفات وعلاقات الأب كاملة قبل الحفظ، بما فيها العلاقات داخل الملف نفسه.</p>
                </div>
            ...
            @if ($preview['rows'] !== [])
                <div class="fd-import-preview-block">
                    <div class="fd-import-preview-bar">
                        <div class="fd-import-preview-heading">معاينة البيانات</div>
                    ...
                    <div class="fd-import-table-wrap">
                        <table class="fd-import-table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="fd-col-code">الرمز</th>
                                    <th>اسم التصنيف</th>
                                    <th class="fd-col-parent">رمز الأب</th>
                                    <th class="fd-col-sort">الترتيب</th>
                            ...
                            <tbody>
                                @foreach ($visibleRows as $row)
                                    <tr>
                                        ...
                                        <td><span class="fd-code">{{ $row['code'] }}</span></td>
                                        <td>{{ $row['name_ar'] }}</td>
                                        <td>
                                            @if ($row['parent_code'])
                                                <span class="fd-code">{{ $row['parent_code'] }}</span>
                                            @else
                                                —
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            @if ($row['status'] === 'active')
                                                <span class="fd-status is-active">فعال</span>
                                            @elseif ($row['status'] === 'inactive')
                                                <span class="fd-status is-inactive">غير فعال</span>
                                            @else
                                                <span class="fd-status is-invalid">{{ $row['status'] ?: 'فارغة' }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        @if ($preview['row_count'] > count($visibleRows))
                            <div class="fd-import-more">
                                يوجد {{ number_format($preview['row_count'] - count($visibleRows)) }} صف إضافي غير ظاهر في المعاينة. تم فحص جميع الصفوف وسيشملها الاستيراد عند نجاح الملف كاملًا.
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @if ($preview['valid'])
                <div class="fd-import-result is-success">
                    ...
                    <div>
                        <strong>الملف صالح للاستيراد.</strong><br>
                        ستتم إضافة شجرة التصنيفات ضمن عملية واحدة، مع حل parent_code بأمان حتى لو جاء الابن قبل الأب داخل الملف.
                    </div>
                </div>
            @else
                    ...
                    <div>
                        ...
                        @if ($errorCount > 12)
                            <div style="margin-top: 7px;">
                                يوجد {{ number_format($errorCount - 12) }} خطأ إضافيًا. أصلح الأخطاء الظاهرة وأعد رفع الملف لمتابعة الفحص.
                            </div>
                        @endif
                    </div>
                </div>
