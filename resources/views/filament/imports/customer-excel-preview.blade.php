@php
    ...
    if ($uploadedFile instanceof TemporaryUploadedFile) {
        $preview = app(CustomerExcelImportService::class)->analyze(
...
@if ($preview !== null)
    ...
    <style>
        ...
        .dark .fd-customer-import-preview {
        ...
        .fd-customer-import-preview * { box-sizing: border-box; }
        ...
        .fd-import-table { width: 100%; min-width: 1480px; border-collapse: collapse; }
    ...
    <div class="fd-customer-import-preview">
        <div class="fd-import-shell">
            <div class="fd-import-head">
                <div>
                    <h3 class="fd-import-title">معاينة ملف العملاء</h3>
                    <p class="fd-import-subtitle">يتم التحقق من البيانات الأساسية وتطابق المنطقة مع خط التوزيع والسياسة الائتمانية قبل السماح بالاستيراد.</p>
                </div>
                            ...
                            <tbody>
                                @foreach ($visibleRows as $row)
                                    <tr>
                                        ...
                                        <td>{{ $row['name'] }}</td>
                                        <td class="fd-ltr">{{ $row['customer_type'] }}</td>
                                        <td class="fd-ltr">{{ $row['phone'] ?? '—' }}</td>
                                        <td class="fd-ltr">{{ $row['mobile'] ?? '—' }}</td>
                                        <td class="fd-ltr">{{ $row['area_code'] ?? '—' }}</td>
                                        <td class="fd-ltr">{{ $row['route_code'] ?? '—' }}</td>
                                        <td class="fd-ltr">{{ is_numeric($row['credit_limit']) ? number_format((float) $row['credit_limit'], 2) : $row['credit_limit'] }}</td>
                                        <td class="fd-ltr">{{ $row['credit_days'] }}</td>
                                        <td class="fd-ltr">{{ $row['payment_type'] }}</td>
                                        <td>
            ...
            <div class="fd-import-policy">
                <div>
                    ...
                    <span>وجود خطأ واحد في أي صف يمنع استيراد الملف كاملًا. عند صلاحية الملف يعاد التحقق مرة أخرى داخل معاملة قاعدة بيانات قبل إنشاء العملاء.</span>
                </div>
            </div>
        </div>
    </div>
